...
void sys_free_user_mem(uint32 virtual_address, uint32 size){
	...
	if ((void*)virtual_address==NULL || virtual_address<USER_HEAP_START || virtual_address>USER_HEAP_MAX){
...
uint32 sys_get_page_permissions(uint32 va)
{

	return pt_get_page_permissions(cur_env->env_page_directory, va);
}

struct FrameInfo* sys_get_frame_info( uint32 virtual_address, uint32 **ptr_page_table)
{
     return get_frame_info(cur_env->env_page_directory, virtual_address, ptr_page_table);
}

uint32 sys_to_frame_number(struct FrameInfo *ptr_frame_info)
{
	return to_frame_number(ptr_frame_info);
}
...
struct Env* sys_get_cur_env()
{
	return cur_env;
}

//uint32 sys_enqueue_env(uint32 sem_queue)
//{
//	enqueue((struct Env_Queue*)sem_queue, cur_env);
//	//acquire_spinlock(&ProcessQueues.qlock);
//	return (uint32)cur_env;
//}
void sys_enqueue_env(uint32 sem_queue, uint32 sem)
{
	acquire_spinlock(&(ProcessQueues.qlock));
	struct semaphore *sem_ptr = (struct semaphore *)sem;
...
uint32 sys_dequeue_env(uint32 sem_queue)
{
	acquire_spinlock(&(ProcessQueues.qlock));
	struct Env *env  = (struct Env*) dequeue((struct Env_Queue*)sem_queue);
	assert(env != NULL);
		{
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_GET_INFO:
			return (uint32)sys_get_frame_info( a1, (uint32**)a2);
			break;
	case SYS_TO_FRNUM:
			return sys_to_frame_number((struct FrameInfo*)a1);
			break;
	...
	case SYS_GET_CUR_ENV:
		return (uint32)sys_get_cur_env();
		break;


	case SYS_ENQUEUE_ENV:
		sys_enqueue_env(a1, a2);
		return 0;
		break;

	case SYS_DEQUEUE_ENV:
		return sys_dequeue_env(a1);
		break;
