...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands);
	char* mycommand = arguments[0];
	int matched = CMD_INVALID;

	for (int i = 0; i < NUM_OF_COMMANDS; i++) {
	    if (strcmp(commands[i].name, mycommand) == 0) {
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++) {
	    int j = 0, len = strlen(mycommand);
	    for (int s = 0; j < len && s < strlen(commands[i].name); s++) {
	        if (mycommand[j] == commands[i].name[s]) j++;
	    }
	    if (j == len) {
	...
	return (LIST_SIZE(&foundCommands) > 0) ? matched : CMD_INVALID;
}
