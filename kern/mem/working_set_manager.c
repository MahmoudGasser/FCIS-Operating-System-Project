...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	struct WorkingSetElement* New_Element = kmalloc(sizeof(struct WorkingSetElement));

	if(New_Element == NULL){
		panic("There is no enough space");
	}
	else{
		New_Element->virtual_address = virtual_address;
	}
		//e->page_last_WS_element->virtual_address = virtual_address;
	//LIST_INSERT_TAIL(&e->page_WS_list,e->page_last_WS_element);
	//e->page_last_WS_element = e->page_last_WS_element ;
	//env_page_ws_print(e);
	return New_Element;
}
